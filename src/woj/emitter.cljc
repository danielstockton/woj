...
(defn string->utf8-bytes
  "Convert a string to a vector of UTF-8 byte values."
  [^String s]
  #?(:woj (let [n (count s)]
            (loop [i 0 acc []]
              (if (>= i n)
                  ...
                  (recur (inc i)
                         (cond
                           ...
                           (-> acc
                               ...
                               (conj (bit-or 0x80 (bit-and (bit-shift-right cp 6) 0x3F)))
                               (conj (bit-or 0x80 (bit-and cp 0x3F))))))))))
     :default (vec (.getBytes s "UTF-8"))))

(defn bytes->hex-escape
  "Convert a sequence of bytes to a WAT hex escape string like \\61\\62\\63."
  [bytes]
  #?(:woj (let [buf (string-buffer)
                hex "0123456789abcdef"]
            (loop [s (seq bytes)]
              (when s
                (let [b (bit-and (first s) 0xff)]
                  (sb-append-char! buf 92) ;; backslash
                  (sb-append-char! buf (codepoint-at hex (bit-shift-right b 4)))
                  (sb-append-char! buf (codepoint-at hex (bit-and b 15))))
                (recur (next s))))
            (sb->string buf))
     :default (apply str (map (fn [b] (format "\\%02x" (bit-and b 0xff))) bytes))))
...
(defn- extract-fn-name
  ...
  (when-let [m (re-find #"\(func \$[a-zA-Z0-9_>]+" fn-def)]
    ;; m is "(func $name" — drop the "(func " prefix to get "$name"
    (let [s (if (vector? m) (first m) m)]
      (subs s 6))))
  ...
  (let [param-names (set (map (fn [p] (str "$" (munge-name p))) params))]
    (remove (fn [local-decl]
              (when-let [m (re-find #"\(local \$[a-zA-Z0-9_>]+" local-decl)]
                (let [s (if (vector? m) (first m) m)]
                  (param-names (subs s 7)))))  ;; drop "(local " prefix
            locals)))
...
(defn- emit-function
  "Emit a WAT function definition."
  ([name params body export-name] (emit-function name params body export-name false))
  ([name params body export-name has-recur]
...
(defn- emit-closure-function
  ...
  ([name params body captures] (emit-closure-function name params body captures false nil))
  ([name params body captures has-recur] (emit-closure-function name params body captures has-recur nil))
  ([name params body captures has-recur self-name]
   (let [munged (if (string? name) name (munge-name name))
         fn-name (str "$" munged)
         arity (count params)
         ...
         locals (filter-locals-against-params
                 (distinct (binding [*capture-indices* capture-map
                             ...
                             (collect-locals body)))
                 params)
         self-local (when self-name (str "(local $" (munge-name self-name) " anyref)"))
...
;; String builtins
(def string-builtins #{'str1 'str-concat 'name 'subs 'string->mem! 'mem->string 'pr-str1 'char 'str-hash 'codepoint-at 'char-from-codepoint})

;; Float operation builtins
(def float-op-builtins #{'to-float 'to-int 'math-floor 'math-ceil 'math-sqrt 'math-abs 'math-round 'math-random})
...
;; Linear memory builtins (WASM linear memory access for binary codecs)
(def memory-builtins #{'mem-write-byte! 'mem-read-byte
...
;; StringBuffer builtins (mutable growable byte buffer)
(def stringbuffer-builtins #{'string-buffer 'sb-append! 'sb-append-char! 'sb->string})

;; WASI file I/O builtins
(def wasi-builtins #{'wasi-slurp 'wasi-stderr 'wasi-args})
...
(defn- emit-call-dispatch
  ...
  (cond
    ...
    (case fn-name
      ...
      codepoint-at (emit-box [:call "$codepoint_at" (emit (first call-args)) (emit-unbox (emit (second call-args)))])
      char-from-codepoint [:call "$char_from_codepoint" (emit-unbox (emit (first call-args)))])

    ;; Float operation builtins
    (contains? float-op-builtins fn-name)
    (if (= fn-name 'math-random)
      [:struct.new "$Float" [:i32.const 5] [:call "$rand_float"]]
    ...
    (contains? stringbuffer-builtins fn-name)
    (case fn-name
      string-buffer [:call "$string_buffer"]
      sb-append! [:call "$sb_append_BANG_" (emit (first call-args)) (emit (second call-args))]
      sb-append-char! [:call "$sb_append_char_BANG_" (emit (first call-args)) (emit-unbox (emit (second call-args)))]
      sb->string [:call "$sb__GT_string" (emit (first call-args))])

    ;; WASI file I/O
    (contains? wasi-builtins fn-name)
    (case fn-name
      wasi-slurp [:call "$wasi_slurp" (emit (first call-args))]
      wasi-stderr [:call "$wasi_stderr" (emit (first call-args))]
      wasi-args [:call "$wasi_args"])
    ...
    ;; Linear memory operations (for binary codecs)
    (contains? memory-builtins fn-name)
    (case fn-name
      mem-write-byte! [:block [:result :anyref]
                       [:i32.store8 (emit-unbox (emit (first call-args)))
                        (emit-unbox (emit (second call-args)))]
                       [:ref.null :none]]
      mem-read-byte (emit-box [:i32.load8_u (emit-unbox (emit (first call-args)))])
      mem-write-i32-be! [:block [:result :anyref]
                         [:call "$mem_write_i32_be"
                          (emit-unbox (emit (first call-args)))
                          (emit-unbox (emit (second call-args)))]
                         [:ref.null :none]]
      mem-read-i32-be (emit-box [:call "$mem_read_i32_be" (emit-unbox (emit (first call-args)))])
      mem-write-f64-be! [:block [:result :anyref]
                         [:call "$mem_write_f64_be"
                          (emit-unbox (emit (first call-args)))
                          [:call "$to_f64" (emit (second call-args))]]
                         [:ref.null :none]]
      mem-read-f64-be [:struct.new "$Float" [:i32.const 5]
                       [:call "$mem_read_f64_be" (emit-unbox (emit (first call-args)))]]
      mem-hash (emit-box [:call "$mem_hash"
                          (emit-unbox (emit (first call-args)))
                          (emit-unbox (emit (second call-args)))]))
  ...
  (cond
    ...
    (= op :float) (let [v (:val ast)]
                    [:struct.new "$Float" [:i32.const 5]
                     [:f64.const (cond
                                   #?(:clj (Double/isNaN v) :default (not= v v)) ##NaN
                                   (= v ##Inf) ##Inf
                                   (= v ##-Inf) ##-Inf
                                   :else v)]])
          ...
          build-env-init (fn [captures]
                            (if (empty? captures)
                              ...
                              (let [env-size (count captures)
                                    env-sets (map-indexed (fn [i sym] (let [value-code (if (and *capture-indices* (contains? *capture-indices* sym)) [:call "$array_get" [:local.get *closure-env-param*] [:i32.const (get *capture-indices* sym)]] [:local.get (str "$" (munge-name sym))])] [:call "$array_set" [:local.get "$__tmp_env"] [:i32.const i] value-code])) captures)]
                                (into [:block [:result :anyref] [:local.set "$__tmp_env" [:call "$array_new" [:i32.const env-size]]]] (conj (vec env-sets) [:local.get "$__tmp_env"])))))]
      (if multi-arity?
        ...
        (let [counter *fn-counter*
              ...
              gen-variadic-destructure (fn []
                          ...
                          "(unreachable)")
              sorted-fixed (reverse (sort-by :arity fixed-fns))
            ...
            (if (and arities (> (count arities) 1))
              ...
              (let [arity-info (first arities)
                ...
                (emit-function (:name ast) params
                               ...
                               (name (:name ast))
                               (or (:has-recur init) (:has-recur arity-info))))))
          nil)
    ...
    (let [field-name (:field ast)
          ...
          matching-types (for [[type-name {:keys [tag fields]}] *user-types*
                               :let [field-idx (let [fv (mapv name fields)
                                                     cnt (count fv)]
                                                 (loop [i 0]
                                                   (cond (>= i cnt) -1
                                                         (= (nth fv i) field-name) i
                                                         :else (recur (inc i)))))]
                               :when (>= field-idx 0)]
...
(defn generate-satisfies-function
  ...
  (let [munged (munge-name protocol-name)
        ...
        type-tags (vec (filter some? (set (map first (keys impls)))))
...
(defn generate-protocol-impl-closures
  ...
  (for [[impl-key impl-fn-name] impls
        :when (and impl-key (some? (nth impl-key 0)))]
...
(defn str-replace-once
  "Replace first occurrence of target in s with replacement.
   Uses str-index-of + StringBuffer for O(N) in woj, string/replace for JVM."
  [s target replacement]
  #?(:woj (let [i (str-index-of s target)]
            (if (neg? i)
              s ;; not found, return original
              (let [tlen (count target)
                    slen (count s)
                    buf (string-buffer)]
                (when (pos? i) (sb-append! buf (subs s 0 i)))
                (sb-append! buf replacement)
                (let [after (+ i tlen)]
                  (when (< after slen) (sb-append! buf (subs s after slen))))
                (sb->string buf))))
     :default (string/replace s target replacement)))

(defn emit-prelude []
  (let [host-import-section (if (seq *host-imports*)
                              (str "\n  ;; Host imports\n  " (str-join "\n  " *host-imports*))
                              "")
        base (str (emit-prelude-1) (emit-user-type-defs) (emit-prelude-2) (emit-prelude-2a) (emit-prelude-transient) (emit-prelude-2b) (emit-prelude-3) (emit-prelude-3b) (emit-prelude-3d) (emit-prelude-sb) (emit-prelude-3c))]
    ;; Replace all placeholders
    (-> base
        (str-replace-once "__HOST_IMPORTS__" host-import-section)
        (str-replace-once "__USER_TYPE_TAGS__"
                          (if (empty? *user-types*) "(i32.const -1)" (emit-user-type-tag-branches)))
        (str-replace-once "__USER_TYPE_EQ__"
                          (emit-user-type-eq-code))
        (str-replace-once "__USER_TYPE_HASH__"
                          (emit-user-type-hash-code))
        (str-replace-once "__USER_COUNTED__"
                          (emit-user-predicate-checks (symbol "-count")))
        (str-replace-once "__USER_MAP__"
                          (emit-user-predicate-checks (symbol "-assoc"))))))

(defn emit-forms
  "Emit AST forms to WAT strings. Pre-scans for internal fn names, then emits.
   Side-effects: populates *functions*, *globals-emit*, *closure-funcs*,
   *lifted-fn-inits*, *fn-refs*, *builtin-refs*, *protocol-impls*, etc.
   Returns {:init-code [...]} with WAT init code strings."
  [ast-forms]
  ...
  {:init-code (into [] (filter some? (map emit-str ast-forms)))})

(defn assemble-module
  "Assemble a complete WAT module from emitted state and optional pre-compiled core fragments.
   Reads dynamic vars populated by emit-forms. ast-forms needed for collect-locals.
   Options:
     :core-functions    - vec of WAT function strings from core
     :core-globals      - vec of WAT global declaration strings from core
     :core-init-code    - vec of WAT init code strings for $start from core
     :core-start-locals - vec of local decl strings for $start from core
     :core-elem-funcs   - vec of function names needing elem declarations from core"
  ([ast-forms init-code] (assemble-module ast-forms init-code {}))
  ([ast-forms init-code opts]
   (let [core-functions (:core-functions opts)
         core-globals (:core-globals opts)
         core-init-code (:core-init-code opts)
         core-start-locals (:core-start-locals opts)
         core-elem-funcs (:core-elem-funcs opts)]
    (let [init-code init-code
                             ...
                             (let [;; Map builtin-arities locally
                                   arities {'inc 1, 'dec 1, 'not 1, 'neg? 1, 'pos? 1, 'zero? 1, 'NaN? 1,
                                         ...
                                         ;; Float operations
                                            ...
                                            'math-abs 1, 'math-round 1, 'math-random 0,
                                         ...
                                         ;; Memory operations
                                            ...
                                            'mem-hash 2,
                                         ;; StringBuffer operations
                                            'string-buffer 0, 'sb-append! 2, 'sb-append-char! 2, 'sb->string 1,
                                         ;; Timing
                                            'now-ms 0}]
                               (for [builtin *builtin-refs*
                                     :when (and builtin (get arities builtin))]
          ...
          proto-satisfies-funcs (when (seq *protocols*)
                                  (for [[proto-name _] *protocols*
                                        :let [;; Get all impls for methods of this protocol
                                              ...
                                              proto-impls (into {} (filter (fn [entry] (contains? proto-methods (nth (first entry) 1))) *protocol-impls*))]]
          ...
          kw-data-info (if (pos? kw-count)
                         (reduce (fn [acc [kw id]]
                                   (let [kw-name (if (namespace kw)
                                                   ...
                                                   (name kw))
                                         bytes (string->utf8-bytes kw-name)
                                         offset (:offset acc)]
                                     (-> acc
                                         (update :bytes (fn [b] (into (or b []) bytes)))
                                         (assoc-in [:entries id] {:offset offset :length (count bytes)})
        ...
        ;; Build data segment for symbol name/ns strings
          sym-data-info (reduce (fn [acc [sym id]]
                                  (let [sym-name (name sym)
                                        ...
                                        ;; Store name bytes
                                        name-bytes (string->utf8-bytes sym-name)
                                    ...
                                    (if sym-ns
                                      (let [ns-bytes (string->utf8-bytes sym-ns)
        ...
        ;; Extract dispatch table info from protocol dispatch maps
          ...
          all-globals (concat core-globals *globals-emit* builtin-globals fn-globals proto-globals
        ...
        ;; Add builtin and fn wrapper functions
          builtin-funcs (when (seq builtin-wrappers)
                          (keep :func-def builtin-wrappers))
          fn-funcs (when (seq fn-wrappers)
                     (keep :func-def fn-wrappers))
  ...
  ;; wasi_slurp: read entire file contents as a String
  ;; Uses WASI preview1: path_open, fd_filestat_get, fd_read, fd_close
  ;; Memory layout (offset 16384+):
  ;;   [16384..16387] = fd result from path_open
  ;;   [16388..16395] = iov struct (base, len)
  ;;   [16396..16399] = nread result
  ;;   [16400..17423] = filename (up to 1024 bytes)
  ;;   [17424..17487] = filestat buffer (64 bytes)
  ;;   [17488..] = file read buffer
  (func $wasi_slurp (param $filename anyref) (result anyref)
    (local $name_len i32)
    (local $fd i32)
    (local $errno i32)
    (local $fsize i32)
    ...
    (local $nread i32)
    (local $remain i32)
    ;; Copy filename to linear memory at 16400
    (local.set $name_len (call $string__GT_mem_BANG_ (local.get $filename) (i32.const 16400)))
    (if (i32.eqz (local.get $name_len))
      (then (return (ref.null none))))
    ;; path_open(dirfd=3, dirflags=1, path=16400, path_len, oflags=0,
    ;;           rights_base=fd_read|fd_seek|fd_filestat_get, rights_inheriting=0, fdflags=0, &fd=16384)
    (local.set $errno (call $path_open
      (i32.const 3)         ;; dirfd: preopened directory
      (i32.const 1)         ;; dirflags: SYMLINK_FOLLOW
      (i32.const 16400)     ;; path pointer
      (local.get $name_len) ;; path length
      (i32.const 0)         ;; oflags: none
      (i64.const 42)        ;; rights_base: fd_read(2) | fd_seek(8) | fd_filestat_get(32) = 42
      (i64.const 0)         ;; rights_inheriting
      (i32.const 0)         ;; fdflags
      (i32.const 16384)))   ;; fd result pointer
    (if (i32.ne (local.get $errno) (i32.const 0))
      (then (return (ref.null none))))
    (local.set $fd (i32.load (i32.const 16384)))
    ;; fd_filestat_get to determine file size
    ;; filestat struct at 17424 (64 bytes): dev(8), ino(8), filetype(1)+pad(7), nlink(8), size(8@offset32), ...
    (local.set $errno (call $fd_filestat_get (local.get $fd) (i32.const 17424)))
    (if (i32.ne (local.get $errno) (i32.const 0))
      (then
        (drop (call $fd_close (local.get $fd)))
        (return (ref.null none))))
    ;; File size at offset 17424+32 = 17456 (i64, take low 32 bits)
    (local.set $fsize (i32.wrap_i64 (i64.load (i32.const 17456))))
    ;; Cap at reasonable max (1MB - 17488 = ~1030000 bytes)
    (if (i32.gt_u (local.get $fsize) (i32.const 1030000))
      (then
        (drop (call $fd_close (local.get $fd)))
        (return (ref.null none))))
    ;; Read file data starting at offset 17488
    (local.set $total_read (i32.const 0))
    (block $read_done
      (loop $read_loop
        (local.set $remain (i32.sub (local.get $fsize) (local.get $total_read)))
        (br_if $read_done (i32.le_s (local.get $remain) (i32.const 0)))
        ;; Setup iov: base=17488+total_read, len=remain
        (i32.store (i32.const 16388) (i32.add (i32.const 17488) (local.get $total_read)))
        (i32.store (i32.const 16392) (local.get $remain))
        ;; fd_read(fd, &iov=16388, 1, &nread=16396)
        (local.set $errno (call $fd_read (local.get $fd) (i32.const 16388) (i32.const 1) (i32.const 16396)))
        (br_if $read_done (i32.ne (local.get $errno) (i32.const 0)))
        (local.set $nread (i32.load (i32.const 16396)))
        (br_if $read_done (i32.eqz (local.get $nread)))
        (local.set $total_read (i32.add (local.get $total_read) (local.get $nread)))
        (br $read_loop)))
    ;; Close fd
    (drop (call $fd_close (local.get $fd)))
    ;; Convert bytes to String
    (call $mem__GT_string (i32.const 17488) (local.get $total_read)))

  ;; wasi_stderr: write a String to stderr (fd 2) for error messages
  (func $wasi_stderr (param $str anyref) (result anyref)
    (local $chars (ref null $CharArray))
    (local $len i32)
    (local $i i32)
    (if (ref.is_null (local.get $str)) (then (return (ref.null none))))
    (if (i32.eqz (ref.test (ref $String) (local.get $str))) (then (return (ref.null none))))
    (local.set $chars (struct.get $String $data (ref.cast (ref $String) (local.get $str))))
    (local.set $len (array.len (local.get $chars)))
    (if (i32.eqz (local.get $len)) (then (return (ref.null none))))
    (local.set $i (i32.const 0))
    (block $done
      (loop $loop
        (br_if $done (i32.ge_s (local.get $i) (local.get $len)))
    ...
    (i32.store (i32.const 4) (i32.const 12))
    (i32.store (i32.const 8) (local.get $len))
    (drop (call $fd_write (i32.const 2) (i32.const 4) (i32.const 1) (i32.const 0)))
    (ref.null none))

  ;; now_ms: get current monotonic time in milliseconds (for profiling)
  (func $now_ms (result anyref)
    (drop (call $clock_time_get (i32.const 1) (i64.const 1000000) (i32.const 16384)))
    (ref.i31 (i32.wrap_i64 (i64.div_u (i64.load (i32.const 16384)) (i64.const 1000000)))))

  ;; wasi_args: get command-line arguments as a vector of strings
  ;; Uses WASI args_sizes_get and args_get
  ;; Memory layout (reuses 16384+ scratch space):
  ;;   [16384..16387] = argc
  ;;   [16388..16391] = argv_buf_size
  ;;   [16392..] = argv pointers (argc * 4 bytes)
  ;;   [16392 + argc*4 ..] = argv buffer (null-terminated strings)
  (func $wasi_args (result anyref)
    (local $argc i32)
    ...
    (local $result anyref)
    ;; Get sizes first
    (if (i32.ne (call $args_sizes_get (i32.const 16384) (i32.const 16388)) (i32.const 0))
      (then (return (call $empty_vector))))
    (local.set $argc (i32.load (i32.const 16384)))
    (local.set $argv_buf_size (i32.load (i32.const 16388)))
    (if (i32.eqz (local.get $argc))
      (then (return (call $empty_vector))))
    ;; argv pointers at 16392, argv buffer after that
    (local.set $argv_ptrs (i32.const 16392))
    (local.set $argv_buf (i32.add (local.get $argv_ptrs) (i32.mul (local.get $argc) (i32.const 4))))
    ;; Get actual args
    (if (i32.ne (call $args_get (local.get $argv_ptrs) (local.get $argv_buf)) (i32.const 0))
      (then (return (call $empty_vector))))
    ...
    (block $done
      (loop $loop
        ...
        ;; Get pointer to this arg string
        (local.set $str_start (i32.load (i32.add (local.get $argv_ptrs) (i32.mul (local.get $i) (i32.const 4)))))
        ;; Find length (null-terminated)
        (local.set $str_len (i32.const 0))
        (block $len_done
          (loop $len_loop
            (br_if $len_done (i32.eqz (i32.load8_u (i32.add (local.get $str_start) (local.get $str_len)))))
            (local.set $str_len (i32.add (local.get $str_len) (i32.const 1)))
            (br $len_loop)))
        ;; Convert to GC string and conj onto vector
        (local.set $result (call $vector_conj (local.get $result)
          (call $mem__GT_string (local.get $str_start) (local.get $str_len))))
        (local.set $i (i32.add (local.get $i) (i32.const 1)))
        (br $loop)))
    (local.get $result))"]
          all-functions (concat core-functions *functions* builtin-funcs fn-funcs
        ...
        ;; Include builtin and fn wrapper functions, and protocol impl functions
          builtin-func-names (when (seq builtin-wrappers)
                               (keep :func-name builtin-wrappers))
          fn-func-names (when (seq fn-wrappers)
                          (keep :func-name fn-wrappers))
          proto-impl-func-names (when (seq proto-impl-closures)
                                  (keep :func-name proto-impl-closures))
          all-closure-funcs (filter some? (concat core-elem-funcs *closure-funcs* builtin-func-names fn-func-names proto-impl-func-names))
        ...
        ;; Generate initialization code for builtin wrappers
          builtin-init-code (when (seq builtin-wrappers)
                              (for [{:keys [global-name func-name closure-type]} builtin-wrappers
                                    :when (and global-name func-name closure-type)]
        ...
        ;; Combine builtin init, fn init, proto init, dispatch table init, string init, kw name init (no drops needed) with user init
          ...
          start-locals (distinct (concat core-start-locals (mapcat collect-locals ast-forms)))
      ...
      #?(:woj
         (let [buf (string-buffer)] (sb-append! buf prelude) (when (seq all-globals) (sb-append! buf "\n\n  ;; Globals\n  ") (sb-append! buf globals-section)) (when data-segments (sb-append! buf data-segments)) (sb-append! buf elem-section) (when (seq all-functions) (sb-append! buf "\n\n  ;; User functions\n  ") (sb-append! buf functions-section)) (sb-append! buf start-fn) (sb-append! buf "\n)") (sb->string buf))
         :default
         (str prelude (if (seq all-globals) (str "\n\n  ;; Globals\n  " globals-section) "") (or data-segments "") elem-section (if (seq all-functions) (str "\n\n  ;; User functions\n  " functions-section) "") start-fn "\n)"))))))

(defn emit-module
  "Backward-compatible: emit AST forms and assemble into a complete WAT module.
   Binds all dynamic vars, emits, and assembles."
  [ast-forms]
  (binding [*functions* []
            *globals-emit* []
            *globals-declared* #{}
            ...
            *protocol-impls* {}
            *lifted-fn-inits* []]
    (let [{:keys [init-code]} (emit-forms ast-forms)]
      (assemble-module ast-forms init-code))))

#?(:woj
(defn generate-user-extras
  "Generate WAT sections for user-added builtins-as-values, fn wrappers, protocol
   dispatch, and lifted fn inits. Returns a map with :globals, :functions, :elem-funcs,
   :init-code suitable for splicing into stream-module."
  []
  (let [;; Builtin wrappers (builtins used as first-class values)
        builtin-wrappers (when (seq *builtin-refs*)
                           (let [arities {'inc 1, 'dec 1, 'not 1, 'neg? 1, 'pos? 1, 'zero? 1, 'NaN? 1,
                                          'first 1, 'rest 1, 'nil? 1, 'cons? 1, 'count 1, 'vector? 1, 'map? 1,
                                          'list? 1, 'keyword? 1, 'number? 1, 'integer? 1, 'fn? 1,
                                          'coll? 1, 'sequential? 1, 'associative? 1, 'counted? 1, 'indexed? 1,
                                          'true? 1, 'false? 1, 'some? 1,
                             ...
                             (for [builtin *builtin-refs*
                               ...
                               (emit-builtin-wrapper builtin (get arities builtin)))))
        fn-wrappers (when (seq *fn-refs*)
                      (for [fn-name *fn-refs* :when fn-name]
                        (let [fn-info (get *direct-fn-globals* fn-name)
                              is-multi (and (map? fn-info) (or (> (count (:arities fn-info)) 1) (:variadic? fn-info)))]
                          (if is-multi
                            (emit-multi-arity-fn-wrapper fn-name fn-info)
                            (let [arity (if (map? fn-info) (or (first (:arities fn-info)) (:min-arity fn-info) 0) (or fn-info 0))]
                              (emit-fn-wrapper fn-name arity))))))
        builtin-globals (keep :global-decl builtin-wrappers)
        fn-globals (keep :global-decl fn-wrappers)
        builtin-funcs (keep :func-def builtin-wrappers)
        fn-funcs (keep :func-def fn-wrappers)
        builtin-func-names (keep :func-name builtin-wrappers)
        fn-func-names (keep :func-name fn-wrappers)
        builtin-init-code (for [{:keys [global-name func-name closure-type]} builtin-wrappers
                                :when (and global-name func-name closure-type)]
                            (str "(global.set " global-name " (struct.new " closure-type " (i32.const 11) (ref.func " func-name ") (call $array_new (i32.const 0))))"))
        fn-init-code (for [{:keys [global-name func-name closure-type is-multi]} fn-wrappers
                           :when (and global-name func-name closure-type)]
                       (if is-multi
                         (str "(global.set " global-name " (struct.new " closure-type " (i32.const 11) (call $array_new (i32.const 0)) (ref.func " func-name ")))")
                         (str "(global.set " global-name " (struct.new " closure-type " (i32.const 11) (ref.func " func-name ") (call $array_new (i32.const 0))))")))
        all-elem-funcs (filter some? (concat builtin-func-names fn-func-names))]
    {:globals (concat builtin-globals fn-globals)
     :functions (concat builtin-funcs fn-funcs)
     :elem-funcs all-elem-funcs
     :init-code (concat builtin-init-code fn-init-code *lifted-fn-inits*)}))
)

#?(:woj
(defn stream-module
  "Stream WAT output directly to stdout using pre-assembled core WAT sections.
   The core WAT is pre-computed on JVM at snapshot time via assemble-module,
   split into: head (prelude+globals+data+elem+functions), start-header (func decl+locals),
   and start-body (init code). User additions are spliced into the right places.

   Takes user AST forms, user init-code, and a map:
     :core-head-chunks    - vec of WAT strings (prelude+globals+data+elem+core functions)
     :core-start-header   - start function declaration + locals
     :core-start-body     - core init code (without closing parens)
     :user-globals        - optional vec of user global declaration strings
     :user-extra-globals  - optional vec of additional globals (strings, symbols)
     :user-data-segments  - optional vec of data segment strings
     :user-extra-fns      - optional vec of additional function strings (init fns)
     :user-extra-init     - optional vec of additional init code strings
     :user-extra-elem-funcs - optional vec of func names needing elem declarations"
  [ast-forms init-code opts]
  (let [head-chunks (:core-head-chunks opts)
        start-header (:core-start-header opts)
        start-body (:core-start-body opts)
        user-globals (:user-globals opts)
        user-extra-globals (:user-extra-globals opts)
        user-data-segments (:user-data-segments opts)
        user-extra-fns (:user-extra-fns opts)
        user-extra-init (:user-extra-init opts)
        user-extra-elem-funcs (:user-extra-elem-funcs opts)
        ;; User additions from emit-forms
        user-functions *functions*
        user-closure-funcs *closure-funcs*
        user-start-locals (distinct (mapcat collect-locals ast-forms))
        init-with-drops (map (fn [code] (str code "\n    drop")) init-code)]
    ;; 1. Print core head: prelude + globals + data segments + elem + core functions
    (loop [i 0]
      (when (< i (count head-chunks))
        (print-str! (nth head-chunks i))
        (recur (inc i))))
    ;; 2. Print user globals (def globals, string/symbol globals)
    (let [all-user-globals (concat user-globals user-extra-globals)]
      (when (seq all-user-globals)
        (print-str! "\n\n  ;; User globals\n  ")
        (print-str! (str-join "\n  " all-user-globals))))
    ;; 3. Print user data segments (string/keyword/symbol byte arrays)
    (when (seq user-data-segments)
      (print-str! "\n\n  ;; User data segments\n  ")
      (print-str! (str-join "\n  " user-data-segments)))
    ;; 4. Print user elem declarations (for closures + wrapper func refs)
    (let [all-elem-funcs (concat user-closure-funcs user-extra-elem-funcs)]
      (when (seq all-elem-funcs)
        (print-str! "\n\n  ;; User closure declarations\n  (elem declare func ")
        (print-str! (str-join " " all-elem-funcs))
        (print-str! ")")))
    ;; 5. Print user functions (from emit-forms) and extra init fns (string/kw/sym init)
    (let [all-user-fns (concat user-functions user-extra-fns)]
      (when (seq all-user-fns)
        (print-str! "\n\n  ")
        (print-str! (str-join "\n\n  " all-user-fns))))
    ;; 6. Print start function header (func decl + core locals)
    (print-str! start-header)
    ;; 7. Print user start locals (if any)
    (when (seq user-start-locals)
      (print-str! "\n    ")
      (print-str! (str-join "\n    " user-start-locals)))
    ;; 8. Print core start body (core init code)
    (print-str! start-body)
    ;; 9. Print user extra init code (string/kw/sym init)
    (when (seq user-extra-init)
      (print-str! "\n    ")
      (print-str! (str-join "\n    " user-extra-init)))
    ;; 10. Print user init code (from emit-forms)
    (when (seq init-with-drops)
      (print-str! "\n    ")
      (print-str! (str-join "\n    " init-with-drops)))
    ;; 11. Close start function and module
    (print-str! ")\n  (start $start)\n)")
    nil))
)
